node {
    withCredentials([[$class: 'UsernamePasswordMultiBinding',
        ...
        stage('Pull') {
            try {
                git branch: 'main', url: 'https://github.com/jiminity/fisa04-jenkins-text.git'
        ...
        stage('Deploy') {
            sshagent(credentials: ['ec2-flask-server']) {
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@52.78.91.124 "sudo docker rm -f docker-flask"')
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@52.78.91.124 "pwd"')
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@52.78.91.124 "sudo docker run --name docker-flask --env-file .env -e TZ=Asia/Seoul -p 80:80 -d -t ${DOCKER_USER_ID}/fisa-app:${BUILD_NUMBER}"')
            }
